#!/usr/bin/env node
// Build the deployment zip for the /api/tester Lambda and print its path.
//
...
const src = join(root, 'functions', 'tester');

const stage = mkdtempSync(join(tmpdir(), 'ari-tester-fn-'));
const zip = join(stage, 'tester-fn.zip');
