#!/usr/bin/env node
// Build the deployment zip for the /api/contrib Lambda and print its path.
//
// Same shape as package-bugreport-fn.mjs and for the same reason: the function
// is an npm workspace, so its dependencies are hoisted to the repo root and
...
const src = join(root, 'functions', 'contrib');

const stage = mkdtempSync(join(tmpdir(), 'ari-contrib-fn-'));
const zip = join(stage, 'contrib-fn.zip');
