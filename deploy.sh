...
# 7) The /api/tester Lambda. Same reasoning as steps 5 and 6 — infra is owned by
#    infra/provision-tester-api.sh, this only ships code onto a function that
#    already exists.
if aws lambda get-function --function-name "$TESTER_FN" --region "$REGION" >/dev/null 2>&1; then
  ZIP="$("$HERE/scripts/package-tester-fn.mjs")"
  aws lambda update-function-code --function-name "$TESTER_FN" --region "$REGION" \
    --zip-file "fileb://$ZIP" >/dev/null
  aws lambda wait function-updated-v2 --function-name "$TESTER_FN" --region "$REGION"
  rm -f "$ZIP"
